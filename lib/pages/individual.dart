...
class _MyAppState extends State<Demo> {
  ...
  StreamController<List<Int16List>>? recordingDataController2;
  IOSink? sink;
  String? text;
  ...
  Future<void> connectToWebSocket() async {
    // 连接到WebSocket服务器
    channel = IOWebSocketChannel.connect('ws://192.168.110.111:10095');
  ...
  void getJsonMessage(dynamic jsonMsg) {
    ...
    String asrmodel = jsonMsg['mode'];
    bool is_final = jsonMsg['is_final'];
    if (asrmodel == "2pass-offline" || asrmodel == "offline") {
      text = rectxt;
    } else {
      text = text! + rectxt; //.replace(/ +/g,"");
    }
    setState(() {});
    // varArea.value = rec_text;
    // print("offline_text: $asrmodel, $offline_text");
        ...
        children: <Widget>[
          ...
          _isRecording
              ? LinearProgressIndicator(
                  value: 100.0 / 160.0 * (dbLevel.value ?? 1) / 100,
          ...
          // Text(text ??
          //     '以下是您购买番茄和土豆的购买信息的简单表格：\n\n\n\n| 商品 | 单价（元/kg） | 购买重量（kg或斤） | 总价（元） |\n| :--: | :--: | :--: | :--: |\n| 番茄 | 3 | 2kg（或2公斤） | 3 * 2 = 6元 |\n| 土豆 | 2 | 5斤（约合2.5kg） | 2 * 2.5 = 5元 |\n\n总计：6元 + 5元 = 11元\n\n请注意，由于您提供的土豆重量是斤，我假设每斤等于0.5千克进行转换。您可以根据实际情况调整。这个表格仅包含您提供的信息，您可以根据需要进行修改或扩展。'),
        ]);
